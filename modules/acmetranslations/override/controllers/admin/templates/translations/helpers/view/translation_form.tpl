						...
						<div name="{$type}_div" id="{$k}-tpl" style="display:{if isset($missing_translations[$k])}block{else}none{/if}">
							<table class="table">
								...
								{foreach $newLang as $key => $value}{counter}
									<tr>
										<td width="40%" class="sourceTranslation">{$key|stripslashes}</td>
										<td width="2%">=</td>
										<td width="40%" class="destTranslation"> {*todo : md5 is already calculated in AdminTranslationsController*}
										...
										<td width="18%">
											...
											{/if}
											<a href="{$link->getAdminLink('AcmeTranslations')|escape:'htmlall':'utf-8'}&lang={$lang}&type={$type}&theme={$theme}&stringtotranslate={$key|stripslashes}" class="translate btn btn-default pull-right" data-translatelang="{$lang}" data-translate="{$key|stripslashes}" {l mod='acmetranslations'}>{l s='Traduire'}</a>

										</td>
									</tr>
									{*//{$link->getAdminLink('AdminTranslateController')}*}
								{/foreach}
							</table>
							...
							<div class="panel-footer">
								...
								<a name="submitTranslations{$type|ucfirst}" href="{$cancel_url|escape:'html':'UTF-8'}" class="btn btn-default"><i class="process-icon-cancel"></i> {l s='Cancel'}</a>
								{*block pour tout traduire *}
								<a href="{$link->getAdminLink('AcmeTranslations')|escape:'htmlall':'utf-8'}&lang={$lang}&type={$type}&theme={$theme}" class="translateAll btn btn-default pull-right"><i class="process-icon-flag"></i>{l s='Tout traduire'}</a>
								{*block pour tout traduire *}
								<button type="submit" id="{$table}_form_submit_btn" name="submitTranslations{$type|ucfirst}" class="btn btn-default pull-right"><i class="process-icon-save"></i> {l s='Save'}</button>
								<button type="submit" id="{$table}_form_submit_btn" name="submitTranslations{$type|ucfirst}AndStay" class="btn btn-default pull-right"><i class="process-icon-save"></i> {l s='Save and stay'}</button>
							</div>
